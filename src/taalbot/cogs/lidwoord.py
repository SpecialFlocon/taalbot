...
class LidwoordCog(commands.Cog):
    ...
    @commands.command(aliases=['hetde', 'lidwoord'], brief=_("De or het? Use this command to find out!"))
...
""").format(display_article, obj['woord'], ctx.bot.command_prefix, ctx.invoked_with, _('both'))
        ...
        # Two arguments: the user is setting the article of a word
            ...
            if all(not get(author.roles, name=r) for r in [const.ROLE_NAME_STAFF]):
                ...
                return await ctx.send(_("Only team members can change articles."))
